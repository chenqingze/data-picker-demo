...
import {PickerLevelType} from "../data-picker/picker.model";
import {debounceTime, Subject} from "rxjs";

@Component({
  selector: 'app-multi-data-picker',
  templateUrl: './multi-data-picker.component.html',
  styleUrls: ['./multi-data-picker.component.scss']
})
export class MultiDataPickerComponent implements OnInit {

  pickerLevel = PickerLevelType;
  isDataLinkage: boolean = true; // 数据是否联动
  @Input() pickerSize = 1; // multiple picker选择列数
  @Input() displayAttributeName: string = ''; // 显示key名称
  @Input() childrenAttributeName: string = ''; // 子节点属性名称
  @Input() headerNameList: Array<string> = []; //  multiple  picker header显示名称
  @Input() linkageData: Array<any> = []; // multiple picker 联动数据
